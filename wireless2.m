...
u_MWT_average=zeros(T,1);%记录平均值
u_DFC_average=zeros(T,1);
...
while t<=T
    ...
    u_MWT_average(t)=sum(u_MWT(:))/n/k;
    u_DFC_average(t)=sum(u_DFC(:))/n/k;
...
Q_LRU=mean(Q_LRU_1(:))
Q_LCE=mean(Q_LCE_1(:))
r_k=mean(u_MWT_average)
x=0:1:T-1;
plot(x,Q_MWT_average,'-b',x,Q_DFC_average,'-r',x,u_MWT_average,'-sb',x,u_DFC_average,'-*r');
set(gca,'XTick',[0:50:200]) %x轴范围0-200，间隔50
set(gca,'YTick',[0:100:500]) %y轴范围0-500，间隔100
legend('with request dropping queue','without request dropping queue','dkal','djaskhfd');   %右上角标注
xlabel('Time')  %x轴坐标描述
